...
def run_cross_sectional():
    ...
    if not latest_scores.empty:
        ...
        st.info(f"**Rebalance Date:** {last_rebalance.strftime('%Y-%m-%d')} | **Next Rebalance:** {next_rebalance}")

        col1, col2 = st.columns(2)
        with col1:
            st.markdown("**🟢 Top 5 — Buy These**")
            for i, (ticker, score) in enumerate(top5.items(), 1):
                st.markdown(f"{i}. **{ticker}** — {score*100:+.1f}% momentum")
        with col2:
            st.markdown("**🔴 Bottom 5 — Avoid These**")
            bot5 = latest_scores.tail(5).iloc[::-1]
            for i, (ticker, score) in enumerate(bot5.items(), 1):
                st.markdown(f"{i}. **{ticker}** — {score*100:+.1f}% momentum")

        st.caption(
            f"Data source: Yahoo Finance | Last updated: {datetime.today().strftime('%Y-%m-%d')} | "
            "Momentum = {lookback_months}-month return. Long-only mode recommended for Investopedia paper trading."
        )
    ...
    col2.metric("Sortino Ratio", f"{sortino:.2f}" if pd.notna(sortino) else "N/A")
    col3.metric("CAGR", f"{cagr:.2%}")
    col4.metric("Ann. Volatility", f"{volatility:.2%}")
    col5.metric("Max Drawdown", f"{max_drawdown:.2%}")

    spy_cagr = (spy_values[-1] / spy_values[0]) ** (1 / total_years) - 1 if spy_values[0] > 0 else 0
    st.metric("SPY CAGR (benchmark)", f"{spy_cagr:.2%}", delta=f"Strategy outperforms by {(cagr-spy_cagr)*100:.1f}%" if cagr > spy_cagr else f"Strategy underperforms by {(spy_cagr-cagr)*100:.1f}%")
    ...
    st.subheader("📊 Selection Heatmap (Blue = Long, Red = Short)")
...
def _fetch_monthly_from_yf(tickers: dict, start: str = "2005-01-01"):
...
def _backtest_dual(levels, lookback=12, abs_mode="ZERO"):
    ...
    for i, date in enumerate(df.index):
        ...
        if pos != target:
            trades.append({"Date": date, "Position": target,
...
def run_dual_momentum():
    ...
    st.markdown(
        ...
        "international equities, or cash. A simple, evidence-based rotation strategy."
    )
...
**Step 1 — Relative Momentum:** Compare US equities (SPY) vs International equities (ACWX).
Hold whichever has stronger trailing momentum over the lookback period.

**Step 2 — Absolute Momentum:** Check if the relative winner has positive momentum vs the threshold.
If YES → hold it. If NO → switch to Cash/Bonds (defensive position).

**ETF Mapping:**
| Allocation | ETF | Description |
|-----------|-----|-------------|
| US Equities | SPY / VTI | S&P 500 or Total US Market |
| International | ACWX / EFA / VXUS | All-World ex-US |
| Cash/Bonds | IEF / BIL / SGOV | Intermediate bonds or T-bills |

**Why it works:** Momentum is one of the most robust factors in finance. By combining relative and absolute momentum, this strategy avoids both underperforming assets AND bear markets.
""")
    ...
    with c2:
        src = st.radio("Data Source", ["Upload CSV", "Demo (synthetic)", "Fetch with yfinance"],
                       index=2, key="dm_src", horizontal=True)
    ...
    if src == "Upload CSV":
        ...
        st.caption(f"Data fetched up to today: **{datetime.today().strftime('%Y-%m-%d')}**")
        ...
        with st.spinner("Fetching live data..."):
            try:
                levels = _fetch_monthly_from_yf(
                    {"US": us_tkr, "INTL": intl_tkr, "CASH": cash_tkr, "TBILL": tbill_tkr}, start=start)
    ...
    if not res.trades.empty:
        ...
        etf_map   = {"US": us_tkr if yf_block else "SPY/VTI", "INTL": intl_tkr if yf_block else "ACWX/EFA", "CASH": cash_tkr if yf_block else "IEF/BIL"}
        ...
        st.markdown(
            ...
            <div style="font-size:1.6rem;font-weight:700;color:{color};">{icon} Hold {current_position} — {etf}</div>
            <div style="color:#ccc;font-size:0.9rem;margin-top:4px;">Signal as of {signal_date} | Lookback: {lookback} months</div>
            </div>""",
            unsafe_allow_html=True
        )

        col1, col2, col3 = st.columns(3)
        col1.metric("🟢 US Equities", us_tkr if yf_block else "SPY",
                    delta="← Current" if current_position == "US" else "")
        col2.metric("🔵 International", intl_tkr if yf_block else "ACWX",
                    delta="← Current" if current_position == "INTL" else "")
        col3.metric("🔴 Cash/Bonds", cash_tkr if yf_block else "IEF",
                    delta="← Current" if current_position == "CASH" else "")

        # Next rebalance
        last_date = res.trades.index[-1]
        next_rebalance = (last_date + pd.offsets.MonthEnd(1)).strftime("%Y-%m-%d")
        st.caption(f"Next rebalance date: **{next_rebalance}**")
    else:
